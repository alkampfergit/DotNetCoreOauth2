...
namespace DotNetCoreOAuth2.Tests
{
    public class Tests
    {
        ...
        public void OneTimeSetup()
        {
            ...
            _sut = new OAuth2Client(_cfh, wkch, "https://foo.com", "client-id");
        }
        
        [Test]
        public async Task Basic_url_generation()
        {
            var url = await _sut.GenerateUrlForCodeFlowAsync("offline_access test", "https://redirecturl", null);
            Assert.That(url.AbsoluteUri.Contains("client_id=client-id"));
            Assert.That(url.AbsoluteUri.Contains("scope=offline_access%20test"));
            Assert.That(url.AbsoluteUri.Contains($"redirect_uri={WebUtility.UrlEncode("https://redirecturl")}"));
        }

        [Test]
        public async Task Uses_well_known_configuration_handler()
        {
            var url = await _sut.GenerateUrlForCodeFlowAsync("offline_access test", "http://redirecturl", null);
        ...
        public async Task Generate_request_for_token_base() 
        {
            ...
            Assert.That(request.Request, Is.Not.Null);
            var realContent = await request.Request.Content!.ReadAsStringAsync();
            Assert.That(realContent.Contains($"client_id=client-id"));
